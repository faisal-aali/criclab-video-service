...
Admin edits to the website `drills.json` do not auto-sync here. A later Mongo catalog can unify them (out of scope for this task). GitHub Actions deploy for this repo is FEAT-027 (Done): push `main` → CI `compileall` → self-hosted `pull.sh` + `restart.sh` (one `criclab-video-worker`).
